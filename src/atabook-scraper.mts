...
export async function getAtabookStatus({url, n, tq = "", eq = "", verbose = true}: AtabookParameters) {
	...
	try {
		...
		// invalid pages redirect to atabook.org/, so checks if it starts with this 
		if (page.status === 204) {
			throw new Error("Site is empty.");
		} else if (page.url.startsWith("https://atabook.org")) { 
			throw new Error("Site was not found. Did you input the correct link?");
		}
		else {
			...
			const cheerioOutput = messages.map((i, element) => {
				...
				const statusQAArr = $element.find('.question').toArray().map(el => $(el).text());
				// default to empty
				let statusText = "";
				let statusEmoji = "";
...
export async function setConfigJson({url, n = 1, tq = "", eq = "", verbose = false}: AtabookParameters) {
		const config = new Conf({ projectName: 'atabook-discord-status' });
		try {
			const jsonVal = await getAtabookStatus({
				url: url,
				n: n,
				tq: tq,
				eq: eq,
			...
			if (jsonVal !== undefined) {
				console.log("Setting config file to contain most recent message of");
				console.dir(jsonVal);
				config.set("recentMsg", jsonVal); 
			} else {
